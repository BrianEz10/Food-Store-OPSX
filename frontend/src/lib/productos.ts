...
export interface ProductoDetail extends ProductoOut {
  ...
  ingredientes: { id: number; nombre: string; descripcion: string | null; es_alergeno: boolean; es_removible: boolean }[]
}
...
export async function uploadImagen(file: File): Promise<{ secure_url: string; public_id: string }> {
