...
export const fetchAdminUsuarios = async (
  ...
  const { data } = await api.get<AdminUsuarioListResponse>('/api/v1/admin/usuarios', { params });
  return data;
};

export const fetchAdminUsuario = async (id: number): Promise<AdminUsuarioDetailResponse> => {
  const { data } = await api.get<AdminUsuarioDetailResponse>(`/api/v1/admin/usuarios/${id}`);
  return data;
};

export const updateAdminUsuario = async (id: number, payload: AdminUsuarioUpdate): Promise<AdminUsuarioResponse> => {
  const { data } = await api.put<AdminUsuarioResponse>(`/api/v1/admin/usuarios/${id}`, payload);
  return data;
};

export const toggleAdminUsuarioEstado = async (id: number, payload: { activo: boolean }): Promise<AdminUsuarioResponse> => {
  const { data } = await api.patch<AdminUsuarioResponse>(`/api/v1/admin/usuarios/${id}/estado`, payload);
  return data;
};
