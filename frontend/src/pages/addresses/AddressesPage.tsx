...
const Toast: React.FC<{ msg: ToastMsg | null; onClose: () => void }> = ({ msg, onClose }) => {
  ...
  const bg = msg.type === 'success'
    ? 'bg-tertiary-light border-tertiary text-tertiary'
...
const AddressCard: React.FC<{
  ...
  <div className={`relative border rounded-xl p-5 bg-white shadow-sm space-y-1 ${direccion.es_principal ? 'border-primary ring-1 ring-primary' : 'border-gray-200'}`}>
    {direccion.es_principal && (
      <span className="absolute top-3 right-3 text-xs font-semibold text-primary bg-primary-light px-2 py-0.5 rounded-full">
        Predeterminada
      </span>
    )}
    {direccion.alias && (
    ...
    <p className="text-sm text-gray-600">{direccion.ciudad}</p>
    ...
    <div className="flex gap-2 pt-3 flex-wrap">
      ...
      {!direccion.es_principal && (
        <button
          ...
          className="text-xs px-3 py-1.5 border border-primary rounded-md hover:bg-primary-light text-primary transition-colors disabled:opacity-50"
        >
      ...
      <button
        ...
        disabled={isDeleting}
        className="text-xs px-3 py-1.5 border border-error rounded-md hover:bg-error-light text-error transition-colors disabled:opacity-50 ml-auto"
      >
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 space-y-4">
        ...
        >
            ...
            {(field) => (
              <div>
                ...
                <input type="text" value={field.state.value} onBlur={field.handleBlur}
                  ...
                  className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-primary focus:border-primary" />
              </div>
            )}
          </form.Field>
          ...
          <form.Field name="linea1"
            ...
            {(field) => (
              <div>
                <label className="block text-sm font-medium text-gray-700">Dirección <span className="text-error">*</span></label>
                <input type="text" value={field.state.value} onBlur={field.handleBlur}
                  onChange={(e) => field.handleChange(e.target.value)}
                  placeholder="Av. Corrientes 1234"
                  className={`mt-1 block w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-primary focus:border-primary ${field.state.meta.errors.length ? 'border-error' : 'border-gray-300'}`} />
          ...
          <form.Field name="ciudad"
            ...
            {(field) => (
              <div>
                <label className="block text-sm font-medium text-gray-700">Ciudad <span className="text-error">*</span></label>
                <input type="text" value={field.state.value} onBlur={field.handleBlur}
                  onChange={(e) => field.handleChange(e.target.value)}
                  className={`mt-1 block w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-primary focus:border-primary ${field.state.meta.errors.length ? 'border-error' : 'border-gray-300'}`} />
                {field.state.meta.errors.length > 0 && (
                  <p className="mt-1 text-xs text-error">{field.state.meta.errors.join(', ')}</p>
                )}
              </div>
            )}
          </form.Field>
            ...
            <form.Subscribe selector={(s) => [s.canSubmit, s.isSubmitting]}>
              {([canSubmit, isSubmitting]) => (
                <button type="submit" disabled={!canSubmit || isPending}
                  className="flex-1 py-2 bg-primary hover:bg-primary-hover text-white text-sm font-medium rounded-md disabled:opacity-50 transition-colors">
    ...
    <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4">
      ...
      <div className="flex gap-3">
        ...
        <button onClick={onConfirm} disabled={isDeleting}
          className="flex-1 py-2 bg-error hover:bg-error text-white text-sm font-medium rounded-md disabled:opacity-50 transition-colors">
          {isDeleting ? 'Eliminando...' : 'Eliminar'}
        </button>
      </div>
    </div>
