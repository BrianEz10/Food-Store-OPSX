...
interface IngItem { ingrediente_id: number; nombre: string; es_removible: boolean }
...
function ProductoFormModal({ mode, id, onClose, onSave }: { mode: 'create' | 'edit'; id?: number; onClose: () => void; onSave: () => void }) {
    ...
    if (mode === 'edit' && id) {
      fetchProducto(id).then((p) => {
        ...
        setIngItems((p as any).ingredientes?.map((ing: any) => ({
          ingrediente_id: ing.id, nombre: ing.nombre, es_removible: ing.es_removible || false,
        })) || [])
  ...
  const addIng = (ing: Ingrediente) => {
    ...
    setIngItems((prev) => [...prev, { ingrediente_id: ing.id, nombre: ing.nombre, es_removible: false }])
  }

  const toggleRemovible = (id: number) => {
    setIngItems((prev) => prev.map((i) => i.ingrediente_id === id ? { ...i, es_removible: !i.es_removible } : i))
  }
            ...
            ingredientes.map((ing) => {
              ...
              return (
                <div key={ing.id} style={{
                  ...
                  {active && (
                    <label style={{ display: 'flex', alignItems: 'center', gap: 4, color: '#c4c7c7', fontSize: 11, cursor: 'pointer' }}>
                      <input type="checkbox" checked={active.es_removible} onChange={() => toggleRemovible(ing.id)} style={{ accentColor: '#e9c349' }} />
                      Es removible
                    </label>
                  )}
                </div>
        ...
        <label style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 12px', background: '#2a2b1b', border: '1px dashed #444748', cursor: 'pointer', fontSize: 12, color: '#c4c7c7' }}>
          ...
          <input type="file" accept="image/*" style={{ display: 'none' }} disabled={uploading}
            onChange={async (e) => {
              ...
              if (!file) return
              setUploading(true)
              try { const result = await uploadImagen(file); setImagenes((prev) => [...prev, result.secure_url]) }
